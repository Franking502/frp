...
import (
	...
	"github.com/xxx/yyy/test/e2e/mock/server/streamserver"
	"github.com/xxx/yyy/test/e2e/pkg/request"
	"github.com/xxx/yyy/test/e2e/pkg/rpc"
)
...
var _ = ginkgo.Describe("[Feature: TCPMUX httpconnect]", func() {
	...
	ginkgo.It("TCPMux Passthrough", func() {
		...
		framework.NewRequestExpect(f).
			RequestModify(func(r *request.Request) {
				r.Addr("normal.example.com").Proxy(proxyURLWithAuth("", "", vhostPort)).Body([]byte("yyy"))
			}).
			ExpectResp([]byte("yyy")).
			Ensure()
