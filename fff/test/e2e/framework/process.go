...
import (
	...
	flog "github.com/xxx/yyy/pkg/util/log"
	"github.com/xxx/yyy/test/e2e/pkg/process"
)
...
func (f *Framework) RunProcesses(serverTemplates []string, clientTemplates []string) ([]*process.Process, []*process.Process) {
	...
	for i := range serverTemplates {
		path := filepath.Join(f.TempDirectory, fmt.Sprintf("yyy-e2e-server-%d", i))
	...
	for i := range clientTemplates {
		...
		path := filepath.Join(f.TempDirectory, fmt.Sprintf("yyy-e2e-client-%d", i))
...
func (f *Framework) GenerateConfigFile(content string) string {
	f.configFileIndex++
	path := filepath.Join(f.TempDirectory, fmt.Sprintf("yyy-e2e-config-%d", f.configFileIndex))
