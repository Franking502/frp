...
if [ "${FRP_VERSION}" = "" ] ; then
  FRP_VERSION="$(curl -sL https://github.com/xxx/yyy/releases | \
...
if [ "${FRP_VERSION}" = "" ] ; then
  printf "Unable to get latest yyy version. Set FRP_VERSION env var and re-run. For example: export FRP_VERSION=1.0.0"
  exit 1;
fi
...
URL="https://github.com/xxx/yyy/releases/download/v${FRP_VERSION}/${NAME}"
...
printf "\n"
printf "yyy has been successfully downloaded into the %s folder on your system.\n" "$DIR_NAME"
printf "\n"
