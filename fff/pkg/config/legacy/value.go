// Copyright 2020 The yyy Authors
//
...
func RenderContent(in []byte) (out []byte, err error) {
	tmpl, errRet := template.New("yyy").Parse(string(in))
	if errRet != nil {
		err = errRet
		return
	}
...
func GetRenderedConfFromPayload(payload string) (out []byte, err error) {
	out, err = RenderContent([]byte(payload))
	return
}
