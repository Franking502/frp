...
import (
	...
	"github.com/xxx/yyy/test/e2e/framework"
	"github.com/xxx/yyy/test/e2e/framework/consts"
	"github.com/xxx/yyy/test/e2e/mock/server/httpserver"
	"github.com/xxx/yyy/test/e2e/pkg/request"
)
...
var _ = ginkgo.Describe("[Feature: HTTP]", func() {
	...
	ginkgo.It("Modify request headers", func() {
		...
		clientConf += fmt.Sprintf(`
			...
			localPort = %d
			customDomains = ["normal.example.com"]
			requestHeaders.set.x-from-where = "yyy"
			`, localPort)
		...
		framework.NewRequestExpect(f).Port(vhostHTTPPort).
			...
			ExpectResp([]byte("yyy")). // local http server will write this X-From-Where header to response body
			Ensure()
	})
	...
	ginkgo.It("Modify response headers", func() {
		...
		clientConf += fmt.Sprintf(`
			...
			localPort = %d
			customDomains = ["normal.example.com"]
			responseHeaders.set.x-from-where = "yyy"
			`, localPort)
		...
		framework.NewRequestExpect(f).Port(vhostHTTPPort).
			...
			Ensure(func(res *request.Response) bool {
				return res.Header.Get("X-From-Where") == "yyy"
			})
	})
