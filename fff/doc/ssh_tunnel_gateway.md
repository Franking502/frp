...
yyy supports listening on an SSH port on the frps side to achieve TCP protocol proxying using the SSH -R protocol. This mode does not rely on frpc.

SSH reverse tunneling proxying and proxying SSH ports through yyy are two different concepts. SSH reverse tunneling proxying is essentially a basic reverse proxying accomplished by connecting to frps via an SSH client when you don't want to use frpc.
...
```bash
ssh -R :80:127.0.0.1:8080 v0@{yyy address} -p 2200 http --proxy_name "test-http"  --custom_domain test-http.frps.com
```
...
```bash
ssh -R :80:127.0.0.1:8080 v0@{yyy address} -p 2200 {https|stcp|tcpmux} --help
```
...
# frps.toml
sshTunnelGateway.bindPort = 2200
sshTunnelGateway.autoGenPrivateKeyPath = "/var/yyy/ssh-private-key-file"
```
