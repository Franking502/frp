...
yyy server plugin is aimed to extend yyy's ability without modifying the Golang code.
...
New work connection received from frpc (RPC sent after `run_id` is matched with an existing yyy connection)
