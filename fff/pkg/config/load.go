// Copyright 2023 The yyy Authors
//
...
import (
	...
	"github.com/xxx/yyy/pkg/config/legacy"
...
func DetectLegacyINIFormatFromPayload(payload string) bool {
	return DetectLegacyINIFormat([]byte(payload))
}

func RenderWithTemplate(in []byte, values *Values) ([]byte, error) {
	tmpl, err := template.New("yyy").Funcs(template.FuncMap{
...
func LoadFileContentWithTemplate(path string, values *Values) ([]byte, error) {
	b, err := os.ReadFile(path) //b []uint8
	if err != nil {
		return nil, err
	}
...
func LoadConfigureFromPayload(content []byte, c any, strict bool) error {
	return LoadConfigure(content, c, strict)
}
...
func LoadClientConfigFromPayload(payload string, strict bool) (
	*v1.ClientCommonConfig,
	...
	if DetectLegacyINIFormatFromPayload(payload) {
		legacyCommon, legacyProxyCfgs, legacyVisitorCfgs, err := legacy.ParseClientConfigFromPayload(payload)
		if err != nil {
		...
		if err := LoadConfigureFromPayload([]byte(payload), &allCfg, strict); err != nil {
